...
class tool {
	...
	static function toolRender( $text, array $args, Parser $parser, PPFrame $frame ) {
		...
		$ret .= "<td><a href='{$github}/{$user}' target='_blank'>{$user}</a> / <a href='{$github}/{$user}/{$repo}' target='_blank'>{$repo}</a> / <a href='{$github}/{$user}/{$repo}/master/{$file}' target='_blank'>{$file}</a></td>";
		$ret .= '</tr>';
